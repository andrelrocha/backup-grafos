...
class Grafo:
    def __init__(self, num_vertices, ponderado=True):
        """ Inicializa um grafo com matriz de adjacência """
        self.num_vertices = num_vertices
        self.ponderado = ponderado
        self.matriz = np.full((num_vertices + 1, num_vertices + 1), np.inf if ponderado else 0)
    ...
    def adicionar_aresta(self, u, v, peso=1):
        ...
        if self.ponderado:
            ...
            self.matriz[v][u] = peso  # Simetria para grafo não direcionado
        else:
            self.matriz[u][v] = 1
            self.matriz[v][u] = 1  # Simetria
        self.graus[u] += 1
        self.graus[v] += 1
    ...
    def m(self):
        ...
        return sum(1 for i in range(1, self.num_vertices + 1) for j in range(i + 1, self.num_vertices + 1) if self.matriz[i][j] < np.inf)
